  ...
  const handleLanguageChange = async (languageCode: string) => {
    ...
    try {
      ...
      toast.success("Preferência de idioma salva", {
        description: `Idioma de transcrição definido para ${languageName}`
      });
    } catch (error) {
      console.error('Failed to save language preference:', error);
      toast.error("Falha ao salvar a preferência de idioma", {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h4 className="text-sm font-medium text-gray-900">Idioma de Transcrição</h4>
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        {isParakeet && (
          <div className="p-2 bg-amber-50 border border-amber-200 rounded text-amber-800">
            <p className="font-medium">ℹ️ Suporte a Idiomas do Parakeet</p>
            <p className="mt-1 text-xs">Atualmente, o Parakeet só suporta detecção automática de idioma. A seleção manual de idioma não está disponível. Use o Whisper se precisar especificar um idioma específico.</p>
          </div>
        )}
        ...
        <div className="text-xs space-y-2 pt-2">
          ...
          {selectedLanguage === 'auto' && (
            <div className="p-2 bg-yellow-50 border border-yellow-200 rounded text-yellow-800">
              <p className="font-medium">⚠️ A detecção automática pode produzir resultados incorretos</p>
              <p className="mt-1">Para obter a melhor precisão, selecione seu idioma específico (ex.: inglês, espanhol etc.)</p>
            </div>
          )}
          {selectedLanguage === 'auto-translate' && (
            <div className="p-2 bg-blue-50 border border-blue-200 rounded text-blue-800">
              <p className="font-medium">🌐 Modo de Tradução Ativo</p>
              <p className="mt-1">Todo o áudio será traduzido automaticamente para o inglês. Ideal para reuniões multilíngues em que você precisa de saída em inglês.</p>
            </div>
          )}
