...
export const RecordingControls: React.FC<RecordingControlsProps> = ({
  ...
  useEffect(() => {
    const checkTauri = async () => {
      try {
        ...
        alert('Falha ao inicializar a gravação. Verifique o console para mais detalhes.');
      }
    };
    checkTauri();
  }, []);
  ...
  const handleStartRecording = useCallback(async () => {
    ...
    try {
      ...
      if (errorMsg.includes('microphone') || errorMsg.includes('mic') || errorMsg.includes('input')
        || errorMsg.includes('microfone') || errorMsg.includes('entrada')) {
        setDeviceError({
          title: 'Microfone Indisponível',
          message: 'Não foi possível acessar o microfone. Verifique se:\n• O microfone está conectado\n• O aplicativo tem permissão de microfone\n• Nenhum outro aplicativo está usando o microfone'
        });
      } else if (errorMsg.includes('system audio') || errorMsg.includes('speaker') || errorMsg.includes('output')
        || errorMsg.includes('áudio do sistema') || errorMsg.includes('alto-falante') || errorMsg.includes('saída')) {
        setDeviceError({
          title: 'Áudio do Sistema Indisponível',
          message: 'Não foi possível capturar o áudio do sistema. Verifique se:\n• Um dispositivo de áudio virtual (como BlackHole) está instalado\n• O aplicativo tem permissão de gravação de tela (macOS)\n• O áudio do sistema está configurado corretamente'
        });
      } else if (errorMsg.includes('permission') || errorMsg.includes('permiss')) {
        setDeviceError({
          title: 'Permissão Necessária',
          message: 'São necessárias permissões de gravação. Por favor:\n• Conceda acesso ao microfone nas Configurações do Sistema\n• Conceda acesso à gravação de tela para áudio do sistema (macOS)\n• Reinicie o aplicativo após conceder as permissões'
        });
      } else {
        setDeviceError({
          title: 'Falha na Gravação',
          message: 'Não foi possível iniciar a gravação. Verifique as configurações do dispositivo de áudio e tente novamente.'
        });
      }
    }
  ...
  const stopRecordingAction = useCallback(async () => {
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        if (error.message.includes('No recording in progress') || error.message.includes('Nenhuma gravação em andamento')) {
          return;
        }
      } else if (typeof error === 'string' && (error.includes('No recording in progress') || error.includes('Nenhuma gravação em andamento'))) {
        return;
      } else if (error && typeof error === 'object' && 'toString' in error) {
        if (error.toString().includes('No recording in progress') || error.toString().includes('Nenhuma gravação em andamento')) {
          return;
        }
      }
      setIsProcessing(false);
  ...
  const handlePauseRecording = useCallback(async () => {
    ...
    try {
      ...
      alert('Falha ao pausar a gravação. Verifique o console para mais detalhes.');
    } finally {
      setIsPausing(false);
    }
  ...
  const handleResumeRecording = useCallback(async () => {
    ...
    try {
      ...
      alert('Falha ao retomar a gravação. Verifique o console para mais detalhes.');
    } finally {
      setIsResuming(false);
    }
