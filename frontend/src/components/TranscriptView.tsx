  ...
  return (
    <div className="px-4 py-2">
      ...
      {transcripts?.map((transcript, index) => {
        ...
        const displayText = originalWasEmpty && !isStreaming ? '[Silêncio]' : filteredText;
        ...
        const sizerText = cleanStopWords(isStreaming ? streamingTranscript.fullText : transcript.text)
          || (originalWasEmpty && !isStreaming ? '[Silêncio]' : '');
      ...
      {transcripts.length === 0 && (
        <EmptyTranscriptState isRecording={isRecording ?? false} isPaused={isPaused ?? false} />
      )}
    </div>
  );
};
