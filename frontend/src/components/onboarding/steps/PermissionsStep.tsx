...
export function PermissionsStep() {
  ...
  const handleMicrophoneAction = async () => {
    if (permissions.microphone === 'denied') {
      ...
      try {
        ...
        alert('Ative o acesso ao microfone em Preferências do Sistema > Segurança e Privacidade > Microfone');
      }
      return;
    }
  ...
  const handleSystemAudioAction = async () => {
    if (permissions.systemAudio === 'denied') {
      ...
      try {
        ...
        alert('Ative a Captura de Áudio em Configurações do Sistema → Privacidade e Segurança → Captura de Áudio');
      }
      return;
    }
  ...
  return (
    <OnboardingContainer
      ...
      description="O ReunIA precisa de acesso ao seu microfone e ao áudio do sistema para gravar reuniões"
      step={4}
      hideProgress={true}
        ...
        <div className="flex flex-col gap-3 pt-4">
          ...
          {!allPermissionsGranted && (
            <p className="text-xs text-center text-muted-foreground">
              A gravação não funcionará sem permissões. Você pode concedê-las depois nas configurações.
            </p>
          )}
        </div>
      </div>
