...
import { Mic, Sparkles, Check, Loader2, Download, Cloud } from 'lucide-react';
...
export function DownloadProgressStep() {
  ...
  // Groq cloud transcription: when true, no local transcription model is needed
  const [groqConfigured, setGroqConfigured] = useState(false);
  const [groqCheckDone, setGroqCheckDone] = useState(false);
  ...
  const handleRetrySummaryDownload = async () => {
    ...
    try {
      ...
      toast.error('Falha ao tentar baixar o modelo de resumo novamente', {
        description: 'Verifique sua conexão e tente novamente.',
      });
    } finally {
      // Allow retry again after 2 seconds
  ...
  // Check if Groq (cloud) transcription was configured during onboarding
  useEffect(() => {
    invoke('api_get_transcript_config')
      .then((config: any) => {
        if (config && config.provider === 'groq') {
          console.log('[DownloadProgressStep] Groq cloud transcription configured - skipping Parakeet download');
          setGroqConfigured(true);
        }
      })
      .catch((error) => {
        console.warn('[DownloadProgressStep] Failed to check transcript config:', error);
      })
      .finally(() => {
        setGroqCheckDone(true);
      });
  }, []);

  // Start the required transcription model immediately; summary readiness must not block it.
  // Skipped when Groq (cloud) is the transcription provider.
  useEffect(() => {
    if (!groqCheckDone) return;
    if (groqConfigured) {
      console.log('[DownloadProgressStep] Groq mode - no Parakeet download needed');
      return;
    }
  ...
  const handleContinue = async () => {
    // Verify actual model availability (catches state drift) - skipped in Groq mode
    if (!groqConfigured) {
      try {
        ...
        if (actuallyAvailable && !parakeetDownloaded) {
          ...
          toast.error('Mecanismo de transcrição necessário', {
            description: 'Tente baixar novamente antes de continuar.',
          });
          return;
        }
      } catch (error) {
    ...
    const downloadsComplete = (groqConfigured || parakeetState.status === 'completed') &&
      summaryState.status === 'completed';

    // Show toast if downloads still in progress
    if (!downloadsComplete) {
      toast.info('Os downloads continuarão em segundo plano', {
        description: 'Você pode começar a usar o aplicativo. A gravação estará disponível quando o reconhecimento de fala estiver pronto.',
        duration: 5000,
      });
    }
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-5">
      ...
      {state.status === 'error' && state.error && (
        <div className="mt-2 p-3 bg-red-50 border border-red-200 rounded-md">
          ...
          {(title === 'Mecanismo de transcrição' || title === 'Mecanismo de resumo') && (
            <button
              onClick={title === 'Mecanismo de transcrição' ? handleRetryDownload : handleRetrySummaryDownload}
  ...
  return (
    <OnboardingContainer
      ...
      description="Você pode começar a usar o ReunIA após baixar o mecanismo de transcrição."
      step={3}
    ...
    >
      <div className="flex flex-col items-center space-y-6">
        ...
        <div className="w-full max-w-lg space-y-4">
          {groqConfigured ? (
            // Groq cloud transcription - no download needed
            <div className="bg-white rounded-xl border border-ambiental-green/40 p-5">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-ambiental-blue-soft flex items-center justify-center">
                    <Cloud className="w-5 h-5 text-ambiental-blue" />
                  </div>
                  <div>
                    <h3 className="font-medium text-gray-900">Transcrição na nuvem</h3>
                    <p className="text-sm text-gray-500">Groq Whisper · sem download</p>
                  </div>
                </div>
                <div className="w-6 h-6 rounded-full bg-green-100 flex items-center justify-center">
                  <Check className="w-4 h-4 text-green-600" />
        ...
        <AnimatePresence>
          {(groqConfigured || parakeetDownloaded) && !summaryModelDownloaded && (
            <motion.div
        ...
        <div className="w-full max-w-xs">
          <Button
            ...
            disabled={!groqCheckDone || (!groqConfigured && !parakeetDownloaded) || isCompleting}
            className="w-full h-11 bg-gray-900 hover:bg-gray-800 text-white disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {(isCompleting || (!groqConfigured && !parakeetDownloaded)) ? (
