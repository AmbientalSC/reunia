...
export function DeviceSelection({ selectedDevices, onDeviceChange, disabled = false }: DeviceSelectionProps) {
  ...
  const fetchDevices = async () => {
    try {
      ...
      setError('Falha ao carregar dispositivos de áudio. Verifique as configurações de áudio do seu sistema.');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const startAudioLevelMonitoring = async () => {
    try {
      ...
      if (deviceNames.length === 0) {
        setError('Nenhum dispositivo de microfone encontrado para monitorar');
        return;
      }
      ...
      setError('Falha ao iniciar o monitoramento do nível de áudio');
    }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h4 className="text-sm font-medium text-gray-900">Dispositivos de Áudio</h4>
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          ...
          {inputDevices.length === 0 && (
            <p className="text-xs text-gray-500">Nenhum dispositivo de microfone encontrado</p>
          )}
        ...
        <div className="space-y-2">
          ...
          >
            <SelectTrigger id="system-selection" className="w-full">
              <SelectValue placeholder="Selecionar áudio do sistema" />
            </SelectTrigger>
            <SelectContent>
      ...
      <div className="text-xs text-gray-500 space-y-1">
        <p>• <strong>Microfone:</strong> Grava sua voz e o som ambiente</p>
        <p>• <strong>Áudio do Sistema:</strong> Grava o áudio do computador (música, chamadas etc.)</p>
        {isMonitoring && (
          <p>• <strong>Níveis do microfone:</strong> Verde = bom, Amarelo = alto, Vermelho = alto demais</p>
        )}
        {!isMonitoring && inputDevices.length > 0 && (
          <p>• <strong>Dica:</strong> Clique em "Testar microfone" para verificar se o seu microfone está funcionando</p>
        )}
      </div>
    </div>
  );
