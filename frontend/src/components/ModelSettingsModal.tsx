...
export function ModelSettingsModal({
  ...
  const loadBuiltinAiModels = async () => {
    ...
    try {
      ...
      toast.error('Falha ao carregar modelos de IA integrados');
    }
  };
  ...
  const handleSave = async () => {
    ...
    if (modelConfig.provider === 'custom-openai') {
      try {
        ...
        toast.error('Falha ao salvar a configuração personalizada do OpenAI');
        return;
      }
    }
  ...
  const testCustomOpenAIConnection = async () => {
    if (!customOpenAIEndpoint.trim() || !customOpenAIModel.trim()) {
      toast.error('Informe primeiro a URL do endpoint e o nome do modelo');
      return;
    }
  ...
  const downloadRecommendedModel = async () => {
    ...
    if (isDownloading(recommendedModel)) {
      toast.info(`${recommendedModel} já está em download`, {
    ...
    try {
      ...
      if (isOllamaNotInstalledError(errorMsg)) {
        toast.error('Ollama não está instalado', {
          ...
          <div className="flex space-x-2 mt-1">
            ...
            >
              ...
              <SelectContent className="max-h-64 overflow-y-auto">
                <SelectItem value="builtin-ai">IA Integrada (Offline, sem necessidade de API)</SelectItem>
        ...
        {modelConfig.provider === 'custom-openai' && (
          <div className="space-y-4 border-t pt-4">
            ...
            <div>
              ...
              />
              <p className="text-xs text-muted-foreground mt-1">
                Identificador do modelo a ser usado nas requisições
              </p>
            </div>

            <div>
              <Label htmlFor="custom-api-key">Chave da API (opcional)</Label>
              <Input
                id="custom-api-key"
                type="password"
            ...
            <div>
              ...
              {isCustomOpenAIAdvancedOpen && (
                <div className="space-y-3 pl-2 border-l-2 border-muted mt-2">
                  <div>
                    <Label htmlFor="custom-max-tokens">Máx. de Tokens</Label>
                    <Input
            ...
            {!isEndpointSectionCollapsed && (
              <>
                <p className="text-sm text-muted-foreground mt-1 mb-2">
                  Deixe vazio ou informe um endpoint personalizado (ex.: http://x.yy.zz:11434)
                </p>
            ...
            {isLoadingOllama ? (
                  ...
                  <div className="space-y-4">
                    <Alert className="border-orange-500 bg-orange-50">
                      <AlertDescription className="text-orange-800">
                        Ollama não está instalado ou não está em execução. Baixe e instale o Ollama para usar modelos locais.
                      </AlertDescription>
                    </Alert>
                    <Button
                      variant="default"
                    ...
                    <Alert className="mb-4">
                      <AlertDescription>
                        {ollamaEndpointChanged
                          ? 'Endpoint alterado. Clique em "Buscar Modelos" para carregar os modelos do novo endpoint.'
                          : 'Nenhum modelo encontrado. Baixe um modelo recomendado ou clique em "Buscar Modelos" para carregar os modelos Ollama disponíveis.'}
                      </AlertDescription>
                    </Alert>
                    {!ollamaEndpointChanged && (
                      <div className="space-y-3">
              ...
              <ScrollArea className="max-h-[calc(100vh-450px)] overflow-y-auto pr-4">
                {filteredModels.length === 0 ? (
                  <Alert>
                    <AlertDescription>
                      Nenhum modelo encontrado correspondendo a "{searchQuery}". Tente um termo de busca diferente.
                    </AlertDescription>
                  </Alert>
                ) : (
