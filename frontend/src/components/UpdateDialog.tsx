...
export function UpdateDialog({ open, onOpenChange, updateInfo }: UpdateDialogProps) {
    ...
    if (open && updateInfo?.available) {
      ...
      check().then((updateResult) => {
        if (updateResult?.available) {
          ...
          setError('A atualização não está mais disponível');
        }
      }).catch((err) => {
        console.error('Failed to get update object:', err);
        setError('Falha ao preparar a atualização: ' + (err.message || 'Erro desconhecido'));
      });
    } else {
  ...
  const handleDownloadAndInstall = async () => {
    ...
    if (!updateToUse) {
      try {
        ...
        if (updateResult?.available) {
          ...
          setError('Atualização não disponível');
          return;
        }
      } catch (err: any) {
        setError('Falha ao obter a atualização: ' + (err.message || 'Erro desconhecido'));
        return;
      }
    }
    ...
    try {
      ...
      setError(err.message || 'Falha ao baixar ou instalar a atualização');
      setIsDownloading(false);
