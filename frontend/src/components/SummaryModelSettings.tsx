...
export function SummaryModelSettings({ refetchTrigger }: SummaryModelSettingsProps) {
  ...
  const fetchModelConfig = useCallback(async () => {
    try {
      ...
      toast.error('Falha ao carregar as configurações do modelo');
    }
  }, []);
  ...
  const handleSaveModelConfig = async (config: ModelConfig) => {
    try {
      ...
      toast.error('Falha ao salvar as configurações do modelo');
    }
  };
    ...
    <div className='flex flex-col gap-4'>
      <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600">Gerar resumo automaticamente após a conclusão (parada) da reunião</p>
          </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
        ...
        <p className="text-sm text-gray-600 mb-6">
          Configure o modelo de IA usado para gerar resumos de reuniões.
        </p>
