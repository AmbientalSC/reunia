    ...
    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-sm font-medium text-blue-900">
            {isCompleted ? 'Finalizando...' : `Baixando ${modelName}`}
          </span>
        </div>
      </div>
      ...
      {isCompleted && (
        <div className="mt-2 text-xs text-green-700">
          ✓ Download concluído, carregando modelo...
        </div>
      )}
    </div>
  );
    ...
    <div className="bg-gray-50 rounded-lg p-3 text-sm">
      <div className="flex items-center justify-between">
        <span className="text-gray-700">
          📦 {downloadedModels} de {totalModels} modelos disponíveis
        </span>
        <span className="text-gray-600">
          💾 {formatSize(totalSizeMb)} no total
        </span>
      </div>
      {downloadedModels > 0 && (
        <div className="mt-1 text-xs text-green-600">
          ✓ Os modelos rodam localmente - não é necessária internet para a transcrição
        </div>
      )}
    </div>
  );
