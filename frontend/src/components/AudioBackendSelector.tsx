...
export function AudioBackendSelector({
  ...
  useEffect(() => {
    const loadBackends = async () => {
      try {
        ...
        setError('Falha ao carregar as opções de backend');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleBackendChange = async (backendId: string) => {
    try {
      ...
      setError('Falha ao alterar o backend. Tente novamente.');
    }
  };
  ...
  return (
    <div className="space-y-2">
          ...
          {showTooltip && (
            <div className="absolute z-10 left-6 top-0 w-64 p-3 text-xs bg-gray-900 text-white rounded-lg shadow-lg">
              ...
              <p className="mt-2 text-gray-300">
                Experimente diferentes backends para descobrir qual funciona melhor no seu sistema.
              </p>
            </div>
          )}
        </div>
      ...
      <div className="text-xs text-gray-500 space-y-1">
        ...
        <p>• O microfone sempre usa o método padrão</p>
