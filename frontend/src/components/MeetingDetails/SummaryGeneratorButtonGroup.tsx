...
export function SummaryGeneratorButtonGroup({
  ...
  const checkBuiltInAIModelsAndGenerate = async () => {
    ...
    try {
      ...
      if (!selectedModel) {
        toast.error('Nenhum modelo de IA integrada selecionado', {
      ...
      if (!modelInfo) {
        toast.error('Modelo não encontrado', {
          description: `Não foi possível encontrar informações para o modelo: ${selectedModel}`,
          duration: 5000,
        });
        setSettingsDialogOpen(true);
        return;
      ...
      if (status.type === 'not_downloaded') {
        toast.error('Modelo não baixado', {
          description: `${selectedModel} precisa ser baixado antes do uso. Abrindo as configurações do modelo...`,
          duration: 5000,
        });
        setSettingsDialogOpen(true);
        return;
      ...
      if (status.type === 'corrupted') {
        toast.error('Arquivo do modelo corrompido', {
          description: `O arquivo de ${selectedModel} está corrompido. Exclua e baixe novamente.`,
          duration: 7000,
        });
        setSettingsDialogOpen(true);
        return;
      ...
      toast.error('Modelo indisponível', {
        description: 'O modelo selecionado não está pronto para uso',
        duration: 5000,
      });
      setSettingsDialogOpen(true);
      ...
      toast.error('Falha ao verificar o status do modelo', {
  ...
  const checkOllamaModelsAndGenerate = async () => {
    ...
    try {
      ...
      if (!models || models.length === 0) {
        ...
        toast.error(
          'Nenhum modelo Ollama encontrado. Baixe o gemma2:2b nas Configurações do Modelo.',
          { duration: 5000 }
        );
        setSettingsDialogOpen(true);
        return;
      ...
      if (isOllamaNotInstalledError(errorMessage)) {
        ...
        toast.error(
          'Falha ao verificar os modelos Ollama. Verifique se o Ollama está em execução e baixe um modelo.',
          { duration: 5000 }
        );
      }
      setSettingsDialogOpen(true);
    ...
    <ButtonGroup>
      ...
      {isGenerating ? (
        <Button
          ...
          title="Parar geração do resumo"
        >
          <Square className="xl:mr-2" size={18} fill="currentColor" />
          <span className="hidden lg:inline xl:inline">Parar</span>
        </Button>
      ) : (
        <Button
          variant="outline"
          ...
          title={
            isModelConfigLoading
              ...
              : isCheckingModels
                ? 'Verificando modelos...'
                : hasSummary ? 'Regenerar Resumo de IA' : 'Gerar Resumo de IA'
          }
        >
          {isCheckingModels || isModelConfigLoading ? (
            <>
              <Loader2 className="animate-spin xl:mr-2" size={18} />
              <span className="hidden xl:inline">Processando...</span>
            </>
          ) : (
            <>
              <Sparkles className="xl:mr-2" size={18} />
              <span className="hidden lg:inline xl:inline">{hasSummary ? 'Regenerar Resumo' : 'Gerar Resumo'}</span>
            </>
          )}
        </Button>
      )}
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              variant="outline"
              size="sm"
              title="Selecionar modelo de resumo"
            >
              <FileText />
