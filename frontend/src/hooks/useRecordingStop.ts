...
export function useRecordingStop(
  ...
  const handleRecordingStop = useCallback(async (isCallApi: boolean) => {
    ...
    try {
      ...
      setStatus(RecordingStatus.PROCESSING_TRANSCRIPTS, 'Aguardando transcrição...');
      ...
      while (elapsedTime < MAX_WAIT_TIME && !transcriptionComplete) {
        try {
          ...
          if (status.chunks_in_queue > 0) {
            ...
            setStatus(RecordingStatus.PROCESSING_TRANSCRIPTS, `Processando ${status.chunks_in_queue} trechos restantes...`);
          }
      ...
      setStatus(RecordingStatus.PROCESSING_TRANSCRIPTS, 'Esvaziando o buffer de transcrição...');
      flushBuffer();
      ...
      if (isCallApi && transcriptionComplete == true) {
        ...
        setStatus(RecordingStatus.SAVING, 'Salvando reunião no banco de dados...');
        ...
        try {
          const responseData = await storageService.saveMeeting(
            savedMeetingName || meetingTitle || 'Nova Reunião',  // PREFER savedMeetingName (backend source)
            freshTranscripts,
            folderPath
          );
          ...
          if (!meetingId) {
            ...
            throw new Error('Nenhum ID de reunião recebido da operação de salvamento');
          }
          ...
          if (shouldDetectSummaryLanguage) {
            try {
              ...
              toast.warning('Não foi possível detectar o idioma do resumo', {
                description: 'A reunião foi salva, mas o Auto não conseguiu detectar o idioma do resumo.',
              });
            }
          }
          ...
          setStatus(RecordingStatus.ERROR, saveError instanceof Error ? saveError.message : 'Erro desconhecido');
          toast.error('Falha ao salvar a reunião', {
      ...
      setStatus(RecordingStatus.ERROR, error instanceof Error ? error.message : 'Erro desconhecido');
      // isRecording already set to false at function start
      setIsRecordingDisabled(false);
    } finally {
