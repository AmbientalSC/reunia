...
export function useImportAudio({
  ...
  const selectFile = useCallback(async (): Promise<AudioFileInfo | null> => {
    ...
    try {
      ...
      setStatus('error');
      const errorMsg = typeof err === 'string' ? err : (err?.message || String(err) || 'Falha ao validar o arquivo');
      setError(errorMsg);
  ...
  const startImport = useCallback(
    async (
      ...
      try {
        ...
        setStatus('error');
        const errorMsg = typeof err === 'string' ? err : (err?.message || String(err) || 'Falha ao iniciar a importação');
        setError(errorMsg);
