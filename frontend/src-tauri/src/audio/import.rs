...
use crate::audio::transcription::groq_provider::GroqProvider;
...
impl ImportGuard {
    ...
    fn acquire() -> Result<Self, String> {
        if IMPORT_IN_PROGRESS
            ...
            .is_err()
        {
            return Err("Importação já em andamento".to_string());
        }
        Ok(ImportGuard)
    }
}
...
pub fn validate_audio_file(path: &Path) -> Result<AudioFileInfo> {
    // Check file exists
    if !path.exists() {
        return Err(anyhow!("O arquivo não existe: {}", path.display()));
    }
    ...
    if !AUDIO_EXTENSIONS.contains(&extension.as_str()) {
        return Err(anyhow!(
            "Formato não suportado: .{}. Suportados: {}",
            extension,
    ...
    let metadata = std::fs::metadata(path)
        .map_err(|e| anyhow!("Não foi possível ler o arquivo: {}", e))?;
    let size_bytes = metadata.len();

    // Check file size limit
    if size_bytes > MAX_FILE_SIZE_BYTES {
        return Err(anyhow!(
            "Arquivo muito grande: {:.2}GB. O tamanho máximo suportado é {}GB",
...
fn extract_duration_from_metadata(path: &Path) -> Result<f64> {
    ...
    let file = std::fs::File::open(path)
        .map_err(|e| anyhow!("Falha ao abrir o arquivo de áudio: {}", e))?;
    ...
    let probed = symphonia::default::get_probe()
        ...
        .map_err(|e| anyhow!("Falha ao sondar o formato de áudio: {}", e))?;

    let format = probed.format;

    // Find the first audio track. Video tracks (e.g. H.264 in MP4 recordings)
    // have no sample rate, so requiring it naturally selects the audio track.
    use symphonia::core::codecs::CODEC_TYPE_NULL;
    let track = format
        .tracks()
        .iter()
        .find(|t| t.codec_params.codec != CODEC_TYPE_NULL && t.codec_params.sample_rate.is_some())
        .ok_or_else(|| anyhow!("Nenhuma faixa de áudio encontrada no arquivo"))?;

    // Extract duration from metadata
    let sample_rate = track
        .codec_params
        .sample_rate
        .ok_or_else(|| anyhow!("Taxa de amostragem desconhecida"))?;

    let n_frames = track
        .codec_params
        .n_frames
        .ok_or_else(|| anyhow!("Contagem de quadros indisponível nos metadados"))?;
...
pub async fn start_import<R: Runtime>(
    ...
    let use_groq = provider.as_deref() == Some("groq");
    ...
    // Groq is a stateless cloud provider - nothing to unload.
    if !use_groq {
...
async fn run_import<R: Runtime>(
    ...
    if !source.exists() {
        return Err(anyhow!("Arquivo de origem não encontrado: {}", source.display()));
    }
    ...
    emit_progress(&app, "copying", 5, "Criando pasta da reunião...");

    // Check for cancellation
    if IMPORT_CANCELLED.load(Ordering::SeqCst) {
        return Err(anyhow!("Importação cancelada"));
    }
    ...
    emit_progress(&app, "copying", 10, "Copiando arquivo de áudio...");
    ...
    tokio::task::spawn_blocking(move || std::fs::copy(&src, &dst))
        .await
        .map_err(|e| anyhow!("Erro na tarefa de cópia: {}", e))?
        .map_err(|e| anyhow!("Falha ao copiar o arquivo de áudio: {}", e))?;
    ...
    emit_progress(&app, "decoding", 15, "Decodificando arquivo de áudio...");
    ...
    .await
    .map_err(|e| anyhow!("Erro na tarefa de decodificação: {}", e))??;
    ...
    emit_progress(&app, "resampling", 20, "Convertendo formato de áudio...");
    ...
    .await
    .map_err(|e| anyhow!("Erro na tarefa de reamostragem: {}", e))?;
    info!(
        "Converted to 16kHz mono format: {} samples",
        audio_samples.len()
    );

    emit_progress(&app, "vad", 25, "Detectando segmentos de fala...");
        ...
        get_speech_chunks_with_progress(
            ...
            |vad_progress, segments_found| {
                ...
                emit_progress(
                    ...
                    overall_progress,
                    &format!(
                        "Detectando segmentos de fala... {}% ({} encontrados)",
                        vad_progress, segments_found
                    ),
                );
    ...
    .await
    .map_err(|e| anyhow!("A tarefa de VAD entrou em pânico: {}", e))?
    .map_err(|e| anyhow!("Falha no processamento de VAD: {}", e))?;
    ...
    if total_segments == 0 {
        ...
        let _ = app.emit(
            "import-warning",
            ImportWarning {
                warning: "Nenhuma fala detectada no arquivo de áudio".to_string(),
                details: Some(
    ...
    emit_progress(&app, "transcribing", 30, "Carregando mecanismo de transcrição...");

    // Initialize the appropriate engine
    let whisper_engine = if !use_parakeet && !use_groq && total_segments > 0 {
    ...
    let groq_engine = if use_groq && total_segments > 0 {
        Some(get_or_init_groq_provider(&app, model.as_deref()).await?)
    } else {
        None
    };
    ...
    for (i, segment) in processable_segments.iter().enumerate() {
        ...
        let (text, conf) = if use_parakeet {
            ...
            let text = engine
                ...
                .await
                .map_err(|e| anyhow!("Falha na transcrição Parakeet no segmento {}: {}", i, e))?;
            (text, 0.9f32)
        } else if use_groq {
            let provider = groq_engine.as_ref().unwrap();
            let result = provider
                .transcribe(segment.samples.clone(), language.clone())
                .await
                .map_err(|e| anyhow!("Falha na transcrição Groq no segmento {}: {}", i, e))?;
            ...
            let (text, conf, _) = engine
                ...
                .await
                .map_err(|e| anyhow!("Falha na transcrição Whisper no segmento {}: {}", i, e))?;
            (text, conf)
        };
    ...
    emit_progress(&app, "saving", 85, "Criando reunião...");
    ...
    emit_progress(&app, "saving", 90, "Escrevendo arquivos de transcrição...");
    ...
    emit_progress(&app, "complete", 100, "Importação concluída");
...
async fn create_meeting_with_transcripts(
    ...
    // Start transaction
    let mut conn = pool.acquire().await.map_err(|e| anyhow!("Erro no banco de dados: {}", e))?;
    let mut tx = sqlx::Connection::begin(&mut *conn)
        .await
        .map_err(|e| anyhow!("Falha ao iniciar a transação: {}", e))?;
    ...
    .execute(&mut *tx)
    .await
    .map_err(|e| anyhow!("Falha ao criar a reunião: {}", e))?;
    ...
    for segment in segments {
        ...
        .execute(&mut *tx)
        .await
        .map_err(|e| anyhow!("Falha ao inserir a transcrição: {}", e))?;
    }

    tx.commit()
        .await
        .map_err(|e| anyhow!("Falha ao confirmar a transação: {}", e))?;
...
/// Get or initialize the Groq cloud transcription provider for import.
///
/// The API key is read from the transcript settings table (column `groqApiKey`).
/// If `requested_model` is provided, it takes precedence over the default.
async fn get_or_init_groq_provider<R: Runtime>(
    app: &AppHandle<R>,
    requested_model: Option<&str>,
) -> Result<Arc<GroqProvider>> {
    info!("☁️ Initializing Groq provider for import...");
    ...
    let api_key = crate::auth::groq_key::resolve_groq_api_key(&app_state, app_state.db_manager.pool(), true)
        ...
        .ok_or_else(|| {
            anyhow!(
                "Chave de API do Groq não configurada. Vá em Configurações > Transcrição e informe sua chave."
            )
        })?;
    ...
    let model = requested_model
        ...
        .filter(|m| !m.is_empty())
        .unwrap_or_else(|| crate::config::DEFAULT_GROQ_WHISPER_MODEL.to_string());

    info!("☁️ Groq provider ready (model: {})", model);
        ...
        Some(e) => {
            ...
            if needs_load {
                ...
                e.load_model(&target_model)
                    .await
                    .map_err(|e| anyhow!("Falha ao carregar o modelo '{}': {}", target_model, e))?;
            }

            Ok(e)
        }
        None => Err(anyhow!("Mecanismo Whisper não inicializado")),
    }
}
...
async fn get_or_init_parakeet<R: Runtime>(
    ...
    match engine {
        ...
        None => Err(anyhow!("Mecanismo Parakeet não inicializado")),
    }
}
...
async fn get_configured_model<R: Runtime>(app: &AppHandle<R>, provider_type: &str) -> Result<String> {
    ...
    .await
    .map_err(|e| anyhow!("Falha ao consultar a configuração: {}", e))?;
...
pub async fn select_and_validate_audio_command<R: Runtime>(
    ...
    .await
    .map_err(|e| format!("Falha na tarefa do diálogo de arquivo: {}", e))?;
...
pub async fn start_import_audio_command<R: Runtime>(
    ...
    Ok(ImportStarted {
        message: "Importação iniciada".to_string(),
    })
}
...
pub async fn cancel_import_command() -> Result<(), String> {
    if !is_import_in_progress() {
        return Err("Nenhuma importação em andamento".to_string());
    }
    cancel_import();
    Ok(())
}
...
mod tests {
    ...
    fn test_validate_audio_file_wrong_extension() {
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Formato não suportado"));
