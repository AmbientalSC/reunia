...
impl RetranscriptionGuard {
    ...
    fn acquire() -> Result<Self, String> {
        if RETRANSCRIPTION_IN_PROGRESS
            ...
            .is_err()
        {
            return Err("Retranscrição já em andamento".to_string());
        }
...
fn find_audio_file(folder: &Path) -> Result<PathBuf> {
    ...
    Err(anyhow!("Nenhum arquivo de áudio encontrado em: {}", folder.display()))
}
...
async fn run_retranscription<R: Runtime>(
    ...
    emit_progress(&app, &meeting_id, "decoding", 5, "Decodificando arquivo de áudio...");

    // Check for cancellation
    if RETRANSCRIPTION_CANCELLED.load(Ordering::SeqCst) {
        return Err(anyhow!("Retranscrição cancelada"));
    }
    ...
    .await
    .map_err(|e| anyhow!("A tarefa de decodificação entrou em pânico: {}", e))??;
    ...
    emit_progress(&app, &meeting_id, "decoding", 15, "Convertendo formato de áudio...");
    ...
    .await
    .map_err(|e| anyhow!("A tarefa de reamostragem entrou em pânico: {}", e))?;
    info!("Converted to 16kHz mono format: {} samples", audio_samples.len());

    emit_progress(&app, &meeting_id, "vad", 20, "Detectando segmentos de fala...");
        ...
        get_speech_chunks_with_progress(
            ...
            |vad_progress, segments_found| {
                ...
                emit_progress(
                    ...
                    "vad",
                    overall_progress,
                    &format!("Detectando segmentos de fala... {}% ({} encontrados)", vad_progress, segments_found),
                );
    ...
    if total_segments == 0 {
        ...
        return Err(anyhow!("Nenhuma fala detectada no arquivo de áudio"));
    }

    emit_progress(&app, &meeting_id, "transcribing", 25, "Carregando mecanismo de transcrição...");

    // Initialize the appropriate engine once (not per-segment)
    let whisper_engine = if !use_parakeet && !use_groq {
    ...
    emit_progress(&app, &meeting_id, "saving", 80, "Salvando transcrições...");
    ...
    sqlx::query("DELETE FROM transcripts WHERE meeting_id = ?")
        ...
        .execute(&mut *tx)
        .await
        .map_err(|e| anyhow!("Falha ao excluir as transcrições existentes: {}", e))?;
    ...
    emit_progress(&app, &meeting_id, "saving", 90, "Escrevendo arquivos de transcrição...");
    ...
    emit_progress(&app, &meeting_id, "complete", 100, "Retranscrição concluída");
            ...
            if needs_load {
                ...
                match e.load_model(&target_model).await {
                    ...
                    Err(load_err) => {
                        ...
                        Err(anyhow!("Falha ao carregar o modelo Whisper '{}': {}", target_model, load_err))
                    }
                }
            } else {
...
async fn get_configured_whisper_model<R: Runtime>(app: &AppHandle<R>) -> Result<String> {
    ...
    let app_state = app
        ...
        .ok_or_else(|| {
            ...
            anyhow!("Estado do aplicativo indisponível")
        })?;
    ...
    .map_err(|e| {
        ...
        anyhow!("Falha ao consultar a configuração de transcrição: {}", e)
    })?;
    ...
    match result {
        Some((provider, model)) => {
            ...
            if provider == "localWhisper" || provider == "whisper" {
                ...
                Err(anyhow!("A retranscrição requer o Whisper. O provedor atual '{}' não suporta retranscrição com seleção de idioma.", provider))
            }
        },
        None => {
            ...
            if needs_load {
                ...
                match e.load_model(&target_model).await {
                    ...
                    Err(load_err) => {
                        ...
                        Err(anyhow!("Falha ao carregar o modelo Parakeet '{}': {}", target_model, load_err))
                    }
                }
            } else {
...
/// Get or initialize the Groq cloud transcription provider for retranscription.
///
/// The API key is read from the transcript settings table (column `groqApiKey`).
/// If `requested_model` is provided (from the retranscription dialog), it takes
/// precedence; otherwise the default Groq Whisper model is used.
async fn get_or_init_groq_provider<R: Runtime>(
    app: &AppHandle<R>,
    requested_model: Option<&str>,
) -> Result<Arc<GroqProvider>> {
    info!("☁️ Initializing Groq provider for retranscription...");
...
pub async fn start_retranscription_command<R: Runtime>(
    ...
    Ok(RetranscriptionStarted {
        meeting_id,
        message: "Retranscrição iniciada".to_string(),
    })
}
...
pub async fn cancel_retranscription_command() -> Result<(), String> {
    if !is_retranscription_in_progress() {
        return Err("Nenhuma retranscrição em andamento".to_string());
    }
...
mod tests {
    ...
    fn test_find_audio_file_empty_folder() {
        ...
        assert!(result.unwrap_err().to_string().contains("Nenhum arquivo de áudio encontrado"));
    }
