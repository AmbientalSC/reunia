...
pub async fn start_recording_with_meeting_name<R: Runtime>(
    ...
    if current_recording_state {
        return Err("Gravação já em andamento".to_string());
    }
    ...
    if let Err(validation_error) = transcription::validate_transcription_model_ready(&app).await {
        ...
        let _ = app.emit("transcription-error", serde_json::json!({
            ...
            "userMessage": "A gravação não pode ser iniciada: o modelo de transcrição ainda está sendo baixado. Aguarde a conclusão do download.",
            "actionable": false
        }));
                ...
                Err(e) => {
                    ...
                    match default_input_device() {
                        ...
                        Err(default_err) => {
                            ...
                            return Err(format!(
                                "Nenhum dispositivo de microfone disponível. Dispositivo preferido '{}' não encontrado e microfone padrão indisponível: {}",
        ...
        None => {
            ...
            match default_input_device() {
                ...
                Err(e) => {
                    ...
                    return Err(format!("Nenhum dispositivo de microfone disponível: {}", e));
                }
            }
        }
    };
    ...
    let effective_meeting_name = meeting_name.clone().unwrap_or_else(|| {
        // Example: Reunião 2025-10-03_08-25-23
    ...
    app.emit("recording-started", serde_json::json!({
        "message": "Gravação iniciada com sucesso com processamento paralelo",
        "devices": ["Microfone Padrão", "Áudio do Sistema Padrão"],
        "workers": 3
...
pub async fn start_recording_with_devices_and_meeting<R: Runtime>(
    ...
    let mic_device = if let Some(ref name) = mic_device_name {
        Some(Arc::new(parse_audio_device(name).map_err(|e| {
            format!("Dispositivo de microfone inválido '{}': {}", name, e)
        })?))
    } else {
        None
    };
    ...
    let system_device = if let Some(ref name) = system_device_name {
        Some(Arc::new(parse_audio_device(name).map_err(|e| {
            format!("Dispositivo de áudio do sistema inválido '{}': {}", name, e)
        })?))
    } else {
        None
    };
    ...
    // Emit success event
    app.emit("recording-started", serde_json::json!({
        "message": "Gravação iniciada com dispositivos personalizados e processamento paralelo",
        "devices": [
            mic_device_name.unwrap_or_else(|| "Microfone Padrão".to_string()),
            system_device_name.unwrap_or_else(|| "Áudio do Sistema Padrão".to_string())
        ],
        "workers": 3
    })).map_err(|e| e.to_string())?;
...
pub async fn stop_recording<R: Runtime>(
    ...
    let _ = app.emit(
        ...
        serde_json::json!({
            ...
            "message": "Parando a captura de áudio...",
            "progress": 20
        }),
    );
    ...
    match stop_result {
        ...
        Err(e) => {
            ...
            return Err(format!("Falha ao parar os fluxos de áudio: {}", e));
        }
    }
    ...
    let _ = app.emit(
        ...
        serde_json::json!({
            ...
            "message": "Processando blocos de transcrição restantes...",
            "progress": 40
        }),
    );
        ...
        let progress_task = tokio::spawn(async move {
            ...
            loop {
                ...
                let _ = progress_app.emit(
                    ...
                    serde_json::json!({
                        "stage": "processing_transcripts",
                        "message": format!("Processando transcrições... ({}s decorridos)", elapsed),
                        "progress": 40,
                        "detailed": true,
    ...
    let _ = app.emit(
        ...
        serde_json::json!({
            ...
            "message": "Descarregando o modelo de reconhecimento de fala...",
            "progress": 70
        }),
    );
    ...
    let _ = app.emit(
        ...
        serde_json::json!({
            "stage": "finalizing",
            "message": "Finalizando a gravação e limpando recursos...",
            "progress": 90
        }),
    );
    ...
    let _ = app.emit(
        ...
        serde_json::json!({
            "stage": "complete",
            "message": "Gravação interrompida com sucesso",
            "progress": 100
        }),
    );
    ...
    app.emit(
        "recording-stopped",
        serde_json::json!({
            "message": "Gravação interrompida - os dados serão salvos após o processamento de todas as transcrições",
...
pub async fn pause_recording<R: Runtime>(app: AppHandle<R>) -> Result<(), String> {
    ...
    if !IS_RECORDING.load(Ordering::SeqCst) {
        return Err("Nenhuma gravação em andamento".to_string());
    }
    ...
    if let Some(manager) = manager_guard.as_ref() {
        ...
        Err("Nenhum gerenciador de gravação encontrado".to_string())
    }
}
