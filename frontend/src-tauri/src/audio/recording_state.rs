...
impl AudioError {
    ...
    pub fn user_message(&self) -> &'static str {
        match self {
            AudioError::DeviceDisconnected => "O dispositivo de áudio foi desconectado",
            AudioError::StreamFailed => "O fluxo de áudio encontrou um erro",
            AudioError::ProcessingFailed => "Falha no processamento de áudio",
            AudioError::TranscriptionFailed => "Falha na transcrição de fala",
            AudioError::ChannelClosed => "O canal de áudio foi fechado inesperadamente",
            AudioError::InitializationFailed => "Falha ao inicializar o sistema de áudio",
            AudioError::ConfigurationError => "Erro na configuração de áudio",
            AudioError::PermissionDenied => "Permissão de microfone negada",
            AudioError::BufferOverflow => "Estouro do buffer de áudio",
            AudioError::SampleRateUnsupported => "Taxa de amostragem de áudio não suportada",
        }
    }
}
...
impl RecordingState {
    ...
    pub fn pause_recording(&self) -> Result<()> {
        if !self.is_recording() {
            return Err(anyhow::anyhow!("Não é possível pausar quando não há gravação"));
        }
        if self.is_paused() {
            return Err(anyhow::anyhow!("A gravação já está pausada"));
        }
    ...
    pub fn resume_recording(&self) -> Result<()> {
        if !self.is_recording() {
            return Err(anyhow::anyhow!("Não é possível retomar quando não há gravação"));
        }
        if !self.is_paused() {
            return Err(anyhow::anyhow!("A gravação não está pausada"));
        }
    ...
    pub fn send_audio_chunk(&self, chunk: AudioChunk) -> Result<()> {
        ...
        if let Some(sender) = self.audio_sender.lock().unwrap().as_ref() {
            sender.send(chunk).map_err(|_| anyhow::anyhow!("Falha ao enviar o bloco de áudio"))?;
            ...
            Err(anyhow::anyhow!("Pipeline de áudio não pronto - nenhum remetente disponível"))
        }
    }
