...
///
/// MP4 files are tried with Symphonia first (native and fast for audio-only
/// MP4/M4A, e.g. the app's own recordings). If that fails — typically a
/// *video* MP4 whose first track is H.264, which this build cannot decode —
/// the file is re-decoded through FFmpeg with `-vn` to extract only the audio.
pub fn decode_audio_file_with_progress(
    ...
    // FFmpeg pre-conversion for formats Symphonia can't decode (MKV, WebM, WMA).
    if needs_ffmpeg_conversion(path) {
        ...
        return decode_with_symphonia(&temp_path.to_path_buf(), progress_callback.as_ref());
    }

    // Native Symphonia decode (audio-only MP4, M4A, WAV, MP3, FLAC, OGG, AAC)
    match decode_with_symphonia(path, progress_callback.as_ref()) {
        Ok(audio) => Ok(audio),
        Err(symphonia_err) => {
            // Video MP4 (e.g. meeting recording): the first track is usually
            // H.264, which Symphonia can't decode. Fall back to FFmpeg, which
            // strips the video track (-vn) and outputs a temporary WAV.
            let is_mp4 = path
                .extension()
                .and_then(|e| e.to_str())
            ...
            if is_mp4 {
                warn!(
                    "Symphonia could not decode MP4 ({}). Retrying via FFmpeg (video file?)",
                    symphonia_err
                );
                let temp_path = convert_to_wav_with_ffmpeg(path, progress_callback.as_ref())?;
                decode_with_symphonia(&temp_path.to_path_buf(), None)
            } else {
                Err(symphonia_err)
            }
        }
...
/// Decode a file with Symphonia directly (no FFmpeg pre-conversion).
fn decode_with_symphonia(
    decode_path: &Path,
    progress_callback: Option<&ProgressCallback>,
) -> Result<DecodedAudio> {
    // Open the file (may be the original file or a temporary WAV)
    let file = std::fs::File::open(decode_path)
    ...
    // Find the first track we can actually decode. Video files (e.g. MP4 with
    // H.264) put the video track first; since this build has no video codecs,
    // we skip tracks whose decoder cannot be created and pick the audio track.
    let mut selected: Option<(u32, Box<dyn symphonia::core::codecs::Decoder>)> = None;
    ...
    for track in format.tracks() {
            ...
            .make(&track.codec_params, &DecoderOptions::default())
        {
            ...
            Err(e) => {
                debug!(
                    "Skipping track {} (codec {:?}): {}",
                    track.id, track.codec_params.codec, e
                );
            }
        }
    }

    let (track_id, mut decoder) = selected
    ...
    // NOTE: n_frames is often unavailable in MP4/streaming containers, so
    // progress tracking degrades gracefully (no updates until 100%).
