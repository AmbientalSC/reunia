...
pub fn get_default_recordings_folder() -> PathBuf {
    ...
    {
        // Windows: %USERPROFILE%\Music\reunia-recordings
        if let Some(music_dir) = dirs::audio_dir() {
            music_dir.join("reunia-recordings")
        } else {
            // Fallback to Documents if Music folder is not available
            dirs::document_dir()
                .unwrap_or_else(|| PathBuf::from("."))
                .join("reunia-recordings")
        }
    }

    #[cfg(target_os = "macos")]
    {
        // macOS: ~/Movies/reunia-recordings
        if let Some(movies_dir) = dirs::video_dir() {
            movies_dir.join("reunia-recordings")
        } else {
    ...
    {
        // Linux/Others: ~/Documents/reunia-recordings
        dirs::document_dir()
            .unwrap_or_else(|| PathBuf::from("."))
            .join("reunia-recordings")
    }
