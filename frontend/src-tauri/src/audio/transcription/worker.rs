...
pub fn start_transcription_task<R: Runtime>(
    ...
    tokio::spawn(async move {
        ...
        let transcription_engine = match super::engine::get_or_init_transcription_engine(&app).await {
            ...
            Err(e) => {
                ...
                let _ = app.emit("transcription-error", serde_json::json!({
                    "error": e,
                    "userMessage": "A gravação falhou: não foi possível inicializar o reconhecimento de fala. Verifique as configurações do modelo.",
                    "actionable": true
                }));
                return;
            }
                    ...
                    match chunk {
                        Some(chunk) => {
                                    ...
                                    if !transcript.trim().is_empty() && meets_threshold {
                                        ...
                                        if !current_flag {
                                            ...
                                            match app_clone.emit("speech-detected", serde_json::json!({
                                                "message": "Atividade de fala detectada"
                                            })) {
                            ...
                            let _ = app_clone.emit("transcription-progress", serde_json::json!({
                                ...
                                "message": format!("Processando blocos de áudio... ({}/{})", completed, queued)
                            }));
                        }
                        None => {
        ...
        let _ = app.emit("transcription-queue-complete", serde_json::json!({
            ...
            "message": format!("{} blocos enfileirados para processamento - aguardando conclusão", total_chunks_queued)
        }));
            ...
            if final_queued == final_completed {
                ...
                let _ = app.emit(
                    ...
                    serde_json::json!({
                        ...
                        "message": "Alguns blocos de transcrição podem ter sido perdidos durante o encerramento"
                    }),
                );
                break;
            }
                ...
                Err(e) => {
                    ...
                    let _ = app.emit(
                        ...
                        &serde_json::json!({
                            ...
                            "userMessage": format!("Falha na transcrição: {}", transcription_error),
                            "actionable": false
                        }),
                    );
            ...
            match provider.transcribe(speech_samples, language).await {
                ...
                Err(e) => {
                    ...
                    let _ = app.emit(
                        ...
                        &serde_json::json!({
                            "error": e.to_string(),
                            "userMessage": format!("Falha na transcrição: {}", e),
                            "actionable": false
                        }),
                    );
