...
pub async fn validate_transcription_model_ready<R: Runtime>(app: &AppHandle<R>) -> Result<(), String> {
    ...
    match config.provider.as_str() {
        ...
        "groq" => {
            info!("☁️ Validating Groq Whisper configuration (no local model required)...");
            match config.api_key.as_deref() {
                Some(key) if !key.trim().is_empty() => {
                    info!("✅ Groq API key configured (model: {})", config.model);
                    Ok(())
                }
                _ => Err(
...
pub async fn get_or_init_transcription_engine<R: Runtime>(
    ...
    match config.provider.as_str() {
        ...
        "groq" => {
            info!("☁️ Initializing Groq Whisper transcription provider");
            ...
            let model = if config.model.trim().is_empty() {
                crate::config::DEFAULT_GROQ_WHISPER_MODEL.to_string()
            } else {
                config.model
            };

            Ok(TranscriptionEngine::Provider(Arc::new(
                crate::audio::transcription::groq_provider::GroqProvider::new(api_key, model),
            )))
        }
        "localWhisper" | _ => {
