...
pub async fn builtin_ai_list_models<R: Runtime>(
    ...
    let manager = {
        // Ensure manager is initialized
        {
            ...
            if manager_lock.is_none() {
                ...
                init_model_manager(&app)
                    .await
                    .map_err(|e| format!("Falha ao inicializar o gerenciador de modelos: {}", e))?;
            }
        }
        ...
        manager_lock
            .as_ref()
            .ok_or_else(|| "Gerenciador de modelos não inicializado".to_string())?
            .clone()
    };
...
pub async fn builtin_ai_get_available_summary_model<R: Runtime>(
    ...
    manager
        .scan_models()
        .await
        .map_err(|e| format!("Falha ao escanear modelos: {}", e))?;
...
pub async fn init_model_manager_at_startup<R: Runtime>(
    ...
    let manager = ModelManager::new_with_models_dir(Some(models_dir))
        .map_err(|e| format!("Falha ao criar o gerenciador de modelos: {}", e))?;
