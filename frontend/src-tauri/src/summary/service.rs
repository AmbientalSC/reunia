...
/// Which in-flight generations a cancel request actually reached.
/// The caller must only touch `summary_processes` when the main summary was
/// cancelled — cancelling just the chained visual task must never mark an
/// already-completed summary as cancelled.
#[derive(Debug, Clone, Copy)]
pub struct CancelOutcome {
    pub summary_cancelled: bool,
    pub visual_cancelled: bool,
}

impl CancelOutcome {
    pub fn any(&self) -> bool {
        self.summary_cancelled || self.visual_cancelled
    }
}
...
impl SummaryService {
    ...
    /// Cancels the summary generation for a meeting, along with the chained
    /// visual summary generation when one is in flight.
    pub fn cancel_summary(meeting_id: &str) -> CancelOutcome {
        let visual_cancelled = crate::summary::visual::cancel_visual_summary(meeting_id);

        let mut summary_cancelled = false;
        ...
        if !summary_cancelled && !visual_cancelled {
    ...
    pub async fn process_transcript_background<R: tauri::Runtime>(
        ...
        let api_key = if provider == LLMProvider::Ollama || provider == LLMProvider::BuiltInAI || provider == LLMProvider::CustomOpenAI {
            ...
            match SettingsRepository::get_api_key(&pool, &model_provider).await {
                ...
                Ok(None) | Ok(Some(_)) => {
                    let err_msg = format!("Chave de API não encontrada para {}", &model_provider);
                    Self::update_process_failed(&pool, &meeting_id, &err_msg).await;
                    return;
                }
                Err(e) => {
                    let err_msg = format!("Falha ao recuperar a chave de API para {}: {}", &model_provider, e);
                    Self::update_process_failed(&pool, &meeting_id, &err_msg).await;
                    return;
                }
            }
        ...
        let (custom_openai_endpoint, custom_openai_api_key, custom_openai_max_tokens, custom_openai_temperature, custom_openai_top_p) =
            if provider == LLMProvider::CustomOpenAI {
                match SettingsRepository::get_custom_openai_config(&pool).await {
                    ...
                    Ok(None) => {
                        let err_msg = "Provedor Custom OpenAI selecionado, mas nenhuma configuração foi encontrada";
                        Self::update_process_failed(&pool, &meeting_id, err_msg).await;
                        return;
                    }
                    Err(e) => {
                        let err_msg = format!("Falha ao recuperar a configuração do Custom OpenAI: {}", e);
                        Self::update_process_failed(&pool, &meeting_id, &err_msg).await;
                        return;
                    }
                }
        ...
        let template = match templates::get_template(&template_id) {
            ...
            Err(e) => {
                let err_msg = format!("Falha ao carregar o template '{}': {}", template_id, e);
                Self::update_process_failed(&pool, &meeting_id, &err_msg).await;
                return;
            }
        };
        ...
        match result {
            Ok((final_markdown, english_markdown, num_chunks)) => {
                ...
                // Chain the second dispatch: generate the visual summary ("Tela")
                // from the freshly generated markdown, reusing the resolved
                // provider configuration. Runs in its own background task so
                // summary polling completes independently.
                let visual_config = crate::summary::visual::VisualLlmConfig {
                    ...
                    token_budget: token_threshold,
                };
                let visual_language = summary_language
                    .clone()
                    .or_else(|| detected_summary_language.clone());
                let visual_title = extract_meeting_name_from_markdown(&final_markdown)
                    .filter(|n| !n.is_empty());
                // Same title-stripped markdown the manual retry path reads
                // back from summary_processes.result, so both triggers feed
                // the LLM identical input.
                tauri::async_runtime::spawn(
                    crate::summary::visual::generate_visual_summary_background(
                        _app.clone(),
                        pool.clone(),
                        meeting_id.clone(),
            ...
            Err(e) => {
                // Check if error is due to cancellation (error strings are
                // localized, so match both English and Portuguese variants)
