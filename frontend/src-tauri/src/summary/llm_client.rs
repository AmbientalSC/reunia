...
impl LLMProvider {
    ...
    pub fn from_str(s: &str) -> Result<Self, String> {
        match s.to_lowercase().as_str() {
            ...
            _ => Err(format!("Provedor LLM não suportado: {}", s)),
        }
    }
}
...
pub async fn generate_summary(
    ...
    if provider == &LLMProvider::BuiltInAI {
        let app_data_dir = app_data_dir
            .ok_or_else(|| "app_data_dir é obrigatório para o provedor BuiltInAI".to_string())?;
    ...
    let (api_url, mut headers) = match provider {
        ...
        LLMProvider::CustomOpenAI => {
            let endpoint = custom_openai_endpoint
                .ok_or_else(|| "Endpoint Custom OpenAI não configurado".to_string())?;
            (
        ...
        LLMProvider::Claude => {
            ...
            header_map.insert(
                ...
                api_key
                    .parse()
                    .map_err(|_| "Formato de chave de API inválido".to_string())?,
            );
            header_map.insert(
                "anthropic-version",
                "2023-06-01"
                    .parse()
                    .map_err(|_| "Versão anthropic inválida".to_string())?,
            );
    ...
    if provider != &LLMProvider::Claude {
        headers.insert(
            ...
            format!("Bearer {}", api_key)
                .parse()
                .map_err(|_| "Cabeçalho de autorização inválido".to_string())?,
        );
    }
    headers.insert(
        header::CONTENT_TYPE,
        "application/json"
            .parse()
            .map_err(|_| "Tipo de conteúdo inválido".to_string())?,
    );
    ...
    let response = if let Some(token) = cancellation_token {
        tokio::select! {
            result = request_future => {
                result.map_err(|e| {
                    if e.is_timeout() {
                        format!("A requisição ao LLM expirou após 60 segundos")
                    } else {
                        format!("Falha ao enviar a requisição ao LLM: {}", e)
                    }
                })?
            }
            _ = token.cancelled() => {
    ...
    if !response.status().is_success() {
        ...
        return Err(format!("Falha na requisição à API do LLM: {}", error_body));
    }
    ...
    if provider == &LLMProvider::Claude {
        let chat_response = response
            ...
            .await
            .map_err(|e| format!("Falha ao analisar a resposta do LLM: {}", e))?;
        ...
        let content = chat_response
            .content
            .get(0)
            .ok_or("Nenhum conteúdo na resposta do LLM")?
            .text
            .trim();
