...
pub async fn initialize_parallel_processor(
    ...
    let safe_workers = state.system_monitor
        ...
        .await
        .map_err(|e| format!("Falha ao calcular o número seguro de workers: {}", e))?;
    ...
    ).map_err(|e| format!("Falha ao criar o processador paralelo: {}", e))?;
...
pub async fn start_parallel_processing(
    ...
    let chunks: Vec<AudioChunk> = audio_chunks
        ...
        .map_err(|e| format!("Falha ao analisar blocos de áudio: {}", e))?;
    ...
    processor.start_processing(chunks.clone(), model_name.clone())
        .await
        .map_err(|e| format!("Falha ao iniciar o processamento paralelo: {}", e))?;
...
pub async fn get_system_resources(
    ...
    state.system_monitor.refresh_system_info()
        .await
        .map_err(|e| format!("Falha ao atualizar as informações do sistema: {}", e))?;

    let resources = state.system_monitor.get_current_resources()
        .await
        .map_err(|e| format!("Falha ao obter os recursos do sistema: {}", e))?;

    serde_json::to_value(resources)
        .map_err(|e| format!("Falha ao serializar os recursos: {}", e))
}
...
pub async fn check_resource_constraints(
    ...
    let status = state.system_monitor.check_resource_constraints()
        .await
        .map_err(|e| format!("Falha ao verificar as restrições de recursos: {}", e))?;

    serde_json::to_value(status)
        .map_err(|e| format!("Falha ao serializar o status dos recursos: {}", e))
}
...
pub async fn calculate_optimal_workers(
    ...
    state.system_monitor.calculate_safe_worker_count()
        .await
        .map_err(|e| format!("Falha ao calcular o número ideal de workers: {}", e))
}
