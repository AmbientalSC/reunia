...
pub async fn whisper_init() -> Result<(), String> {
    ...
    let engine = WhisperEngine::new_with_models_dir(models_dir)
        .map_err(|e| format!("Falha ao inicializar o mecanismo Whisper: {}", e))?;
...
pub async fn whisper_get_available_models() -> Result<Vec<ModelInfo>, String> {
    ...
    if let Some(engine) = engine {
        engine
            .discover_models()
            .await
            .map_err(|e| format!("Falha ao descobrir modelos: {}", e))
    } else {
...
fn discover_models_standalone() -> Result<Vec<ModelInfo>, String> {
    ...
    let models_dir = get_models_directory()
        .ok_or_else(|| "Diretório de modelos não inicializado".to_string())?;
...
pub async fn whisper_load_model(
    ...
    if let Some(engine) = engine {
        ...
        Err("Mecanismo Whisper não inicializado".to_string())
    }
}
...
pub async fn whisper_validate_model_ready() -> Result<String, String> {
    ...
    if let Some(engine) = engine {
        ...
        if available_models.is_empty() {
            return Err(
                "Nenhum modelo Whisper disponível. Baixe um modelo para habilitar a transcrição."
                    .to_string(),
            );
        }
        ...
        engine
            ...
            .await
            .map_err(|e| format!("Falha ao carregar o modelo {}: {}", first_model.name, e))?;
...
pub async fn whisper_validate_model_ready_with_config<R: tauri::Runtime>(
    ...
    if let Some(engine) = engine {
        ...
        engine
            .load_model(&model_name)
            .await
            .map_err(|e| format!("Falha ao carregar o modelo {}: {}", model_name, e))?;
    ...
    if let Some(engine) = engine {
        ...
        match result {
            ...
            Err(e) => {
                ...
                Err(format!("Falha ao baixar o modelo: {}", e))
            }
        }
    } else {
        Err("Mecanismo Whisper não inicializado".to_string())
...
pub async fn whisper_cancel_download(model_name: String) -> Result<(), String> {
    ...
    if let Some(engine) = engine {
        engine
            ...
            .await
            .map_err(|e| format!("Falha ao cancelar o download: {}", e))
    } else {
        Err("Mecanismo Whisper não inicializado".to_string())
    }
}
...
pub async fn whisper_delete_corrupted_model(model_name: String) -> Result<String, String> {
    ...
    if let Some(engine) = engine {
        engine
            ...
            .await
            .map_err(|e| format!("Falha ao excluir o modelo: {}", e))
    } else {
        Err("Mecanismo Whisper não inicializado".to_string())
    }
}
