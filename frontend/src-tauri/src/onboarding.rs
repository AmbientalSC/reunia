...
pub async fn save_onboarding_status<R: Runtime>(
    ...
    let store = app.store("onboarding-status.json")
        .map_err(|e| anyhow::anyhow!("Falha ao acessar o armazenamento de integração: {}", e))?;
    ...
    let status_value = serde_json::to_value(&status)
        .map_err(|e| anyhow::anyhow!("Falha ao serializar o status de integração: {}", e))?;
    ...
    // Persist to disk
    store.save()
        .map_err(|e| anyhow::anyhow!("Falha ao salvar o armazenamento de integração no disco: {}", e))?;
...
pub async fn reset_onboarding_status<R: Runtime>(
    ...
    store.save()
        .map_err(|e| anyhow::anyhow!("Falha ao salvar o armazenamento de integração após a redefinição: {}", e))?;
...
pub async fn save_onboarding_status_cmd<R: Runtime>(
    ...
    save_onboarding_status(&app, &status)
        .await
        .map_err(|e| format!("Falha ao salvar o status de integração: {}", e))
}
...
pub async fn reset_onboarding_status_cmd<R: Runtime>(
    ...
    reset_onboarding_status(&app)
        .await
        .map_err(|e| format!("Falha ao redefinir o status de integração: {}", e))
}
...
pub async fn complete_onboarding<R: Runtime>(
    ...
    if let Err(e) = SettingsRepository::save_model_config(
        ...
        return Err(format!("Falha ao salvar a configuração do modelo builtin-ai: {}", e));
    }
    info!("Saved builtin-ai model config: model={}", model);

    // Save transcription model config.
    // If the user chose the Groq (cloud) provider during onboarding, keep that
    // config; otherwise default to the local Parakeet model.
    let existing_transcript = SettingsRepository::get_transcript_config(pool)
        .await
        .ok()
        .flatten();
    let uses_groq = matches!(existing_transcript.as_ref(), Some(c) if c.provider == "groq");

    if uses_groq {
        info!("Keeping Groq (cloud) transcription config chosen during onboarding");
    } else if let Err(e) = SettingsRepository::save_transcript_config(
        pool,
        "parakeet",
        crate::config::DEFAULT_PARAKEET_MODEL,
    ).await {
        error!("Failed to save transcription model config: {}", e);
        return Err(format!("Falha ao salvar a configuração do modelo de transcrição: {}", e));
    }
    info!(
        "Transcription config ready: provider={}",
        if uses_groq { "groq" } else { "parakeet" }
    );
    ...
    status.model_status.parakeet = if uses_groq {
        "not_downloaded".to_string() // Cloud transcription - no local model needed
    } else {
    ...
    save_onboarding_status(&app, &status)
        .await
        .map_err(|e| format!("Falha ao salvar o status de integração concluído: {}", e))?;
