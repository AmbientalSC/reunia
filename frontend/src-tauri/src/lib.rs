...
// Global language preference storage. Default to "auto" (detect language,
// never translate): the old "auto-translate" default silently translated
// meetings to English whenever a recording started before the frontend
// synced the user's saved preference.
...
async fn start_recording<R: Runtime>(
    ...
    .await
    {
        ...
        Err(e) => {
            ...
            Err(format!("Falha ao iniciar a gravação: {}", e))
        }
    }
}
...
async fn stop_recording<R: Runtime>(app: AppHandle<R>, args: RecordingArgs) -> Result<(), String> {
    ...
    .await
    {
            ...
            if let Some(parent) = std::path::Path::new(&args.save_path).parent() {
                if !parent.exists() {
                    ...
                    if let Err(e) = std::fs::create_dir_all(parent) {
                        let err_msg = format!("Falha ao criar o diretório de salvamento: {}", e);
                        log_error!("{}", err_msg);
                        return Err(err_msg);
                    }
                }
        ...
        Err(e) => {
            ...
            RECORDING_FLAG.store(false, Ordering::SeqCst);
            tray::update_tray_menu(&app);
            Err(format!("Falha ao parar a gravação: {}", e))
        }
    }
}
...
fn read_audio_file(file_path: String) -> Result<Vec<u8>, String> {
    match std::fs::read(&file_path) {
        Ok(data) => Ok(data),
        Err(e) => Err(format!("Falha ao ler o arquivo de áudio: {}", e)),
    }
}
    ...
    if let Some(parent) = std::path::Path::new(&file_path).parent() {
        if !parent.exists() {
            std::fs::create_dir_all(parent)
                .map_err(|e| format!("Falha ao criar o diretório: {}", e))?;
        }
    }

    // Write content to file
    std::fs::write(&file_path, content)
        .map_err(|e| format!("Falha ao gravar a transcrição: {}", e))?;
...
async fn start_audio_level_monitoring<R: Runtime>(
    ...
    audio::simple_level_monitor::start_monitoring(app, device_names)
        .await
        .map_err(|e| format!("Falha ao iniciar o monitoramento do nível de áudio: {}", e))
}
...
async fn stop_audio_level_monitoring() -> Result<(), String> {
    ...
    audio::simple_level_monitor::stop_monitoring()
        .await
        .map_err(|e| format!("Falha ao parar o monitoramento do nível de áudio: {}", e))
}
...
async fn get_audio_devices() -> Result<Vec<AudioDevice>, String> {
    list_audio_devices()
        .await
        .map_err(|e| format!("Falha ao listar os dispositivos de áudio: {}", e))
}

#[tauri::command]
async fn trigger_microphone_permission() -> Result<bool, String> {
    trigger_audio_permission()
        .map_err(|e| format!("Falha ao solicitar a permissão do microfone: {}", e))
}
...
async fn set_language_preference(language: String) -> Result<(), String> {
    let mut lang_pref = LANGUAGE_PREFERENCE
        .lock()
        .map_err(|e| format!("Falha ao definir a preferência de idioma: {}", e))?;
...
pub fn run() {
    ...
    builder
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            read_audio_file,
            save_transcript,
            auth::commands::login_with_microsoft,
            auth::commands::set_auth_session,
            auth::commands::clear_auth_session,
            ...
            // Visual summary ("Tela") commands
            summary::commands::api_get_visual_summary,
            summary::commands::api_generate_visual_summary,
            // Live insight (real-time, during an active recording)
            summary::commands::api_generate_live_insight,
            // Template commands
