...
pub async fn parakeet_init() -> Result<(), String> {
    ...
    let engine = ParakeetEngine::new_with_models_dir(models_dir)
        .map_err(|e| format!("Falha ao inicializar o mecanismo Parakeet: {}", e))?;
...
pub async fn parakeet_get_available_models() -> Result<Vec<ModelInfo>, String> {
    ...
    if let Some(engine) = engine {
        engine
            .discover_models()
            .await
            .map_err(|e| format!("Falha ao descobrir modelos Parakeet: {}", e))
    } else {
        Err("Mecanismo Parakeet não inicializado".to_string())
    }
}
...
pub async fn parakeet_load_model<R: Runtime>(
    ...
    if let Some(engine) = engine {
        ...
        let result = engine
            ...
            .await
            .map_err(|e| format!("Falha ao carregar o modelo Parakeet: {}", e));
...
pub async fn parakeet_validate_model_ready() -> Result<String, String> {
    ...
    if let Some(engine) = engine {
        ...
        if available_models.is_empty() {
            return Err(
                "Nenhum modelo Parakeet disponível. Baixe um modelo para habilitar a transcrição rápida."
                    .to_string(),
            );
        }
        ...
        engine
            ...
            .await
            .map_err(|e| format!("Falha ao carregar o modelo Parakeet {}: {}", first_model.name, e))?;
...
pub async fn parakeet_validate_model_ready_with_config<R: tauri::Runtime>(
    ...
    if let Some(engine) = engine {
        ...
        engine
            .load_model(&model_name)
            .await
            .map_err(|e| format!("Falha ao carregar o modelo Parakeet {}: {}", model_name, e))?;
...
pub async fn parakeet_transcribe_audio(audio_data: Vec<f32>) -> Result<String, String> {
    ...
    if let Some(engine) = engine {
        engine
            ...
            .await
            .map_err(|e| format!("Falha na transcrição Parakeet: {}", e))
    } else {
        Err("Mecanismo Parakeet não inicializado".to_string())
    }
}
    ...
    if let Some(engine) = engine {
        ...
        match result {
            ...
            Err(e) => {
                ...
                Err(format!("Falha ao baixar o modelo Parakeet: {}", e))
            }
        }
    } else {
        Err("Mecanismo Parakeet não inicializado".to_string())
...
pub async fn parakeet_cancel_download<R: Runtime>(
    ...
    if let Some(engine) = engine {
        engine
            ...
            .await
            .map_err(|e| format!("Falha ao cancelar o download do modelo Parakeet: {}", e))?;
...
pub async fn parakeet_delete_corrupted_model(model_name: String) -> Result<String, String> {
    ...
    if let Some(engine) = engine {
        engine
            ...
            .await
            .map_err(|e| format!("Falha ao excluir o modelo Parakeet: {}", e))
    } else {
        Err("Mecanismo Parakeet não inicializado".to_string())
    }
}
...
pub async fn open_parakeet_models_folder() -> Result<(), String> {
    let models_dir = get_models_directory()
        .ok_or_else(|| "Diretório de modelos Parakeet não inicializado".to_string())?
        .join("parakeet");
