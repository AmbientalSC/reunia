...
async fn make_api_request<R: Runtime, T: for<'de> Deserialize<'de>>(
    ...
    let response_text = response.text().await.map_err(|e| {
        let error_msg = format!("Falha ao ler a resposta: {}", e);
        log_error!("{}", error_msg);
        error_msg
    })?;
    ...
    serde_json::from_str(&response_text).map_err(|e| {
        let error_msg = format!("Falha ao analisar JSON: {}", e);
        log_error!("{}", error_msg);
        error_msg
    })
}
...
pub async fn api_search_transcripts<R: Runtime>(
    ...
    match TranscriptsRepository::search_transcripts(pool, &query).await {
        ...
        Err(e) => {
            ...
            Err(format!("Falha ao pesquisar transcrições: {}", e))
        }
    }
}
...
pub async fn api_save_model_config<R: Runtime>(
    ...
    Ok(
        serde_json::json!({ "status": "success", "message": "Configuração do modelo salva com sucesso" }),
    )
}
...
pub async fn api_save_transcript_config<R: Runtime>(
    ...
    Ok(
        serde_json::json!({ "status": "success", "message": "Configuração de transcrição salva com sucesso" }),
    )
}
...
pub async fn api_delete_meeting<R: Runtime>(
    ...
    match MeetingsRepository::delete_meeting(pool, &meeting_id).await {
        ...
        Err(e) => {
            ...
            Err(format!("Falha ao excluir reunião: {}", e))
        }
    }
}
...
pub async fn api_get_meeting<R: Runtime>(
    ...
    match MeetingsRepository::get_meeting(pool, &meeting_id).await {
        ...
        Ok(None) => {
            ...
            Err(format!("Reunião não encontrada: {}", meeting_id))
        }
        Err(e) => {
            log_error!("Error retrieving meeting {}: {}", meeting_id, e);
            Err(format!("Falha ao recuperar a reunião: {}", e))
        }
    }
}
...
pub async fn api_get_meeting_metadata<R: Runtime>(
    ...
    match MeetingsRepository::get_meeting_metadata(pool, &meeting_id).await {
        ...
        Err(e) => {
            ...
            Err(format!("Falha ao recuperar os metadados da reunião: {}", e))
        }
    }
}
...
pub async fn api_save_meeting_title<R: Runtime>(
    ...
    match MeetingsRepository::update_meeting_title(pool, &meeting_id, &title).await {
        Ok(true) => {
            ...
            Ok(serde_json::json!({"message": "Título da reunião salvo com sucesso"}))
        }
        Ok(false) => {
        ...
        Err(e) => {
            ...
            Err(format!("Falha ao atualizar a reunião: {}", e))
        }
    }
}
...
pub async fn api_save_transcript<R: Runtime>(
    ...
    .await
    {
        ...
        Err(e) => {
            ...
            Err(format!("Falha ao salvar a transcrição: {}", e))
        }
    }
}
        ...
        Some(m) => {
            if let Some(folder_path) = m.folder_path {
                ...
                if !path.exists() {
                    ...
                    return Err(format!("Pasta de gravação não encontrada: {}", folder_path));
                }
                ...
                Err("Caminho da pasta de gravação indisponível para esta reunião".to_string())
            }
        }
        None => {
            log_warn!("Meeting not found: {}", meeting_id);
            Err("Reunião não encontrada".to_string())
        }
    }
}
...
pub async fn test_backend_connection<R: Runtime>(
    ...
    match request.send().await {
        ...
        Err(e) => {
            let error_msg = format!("Falha ao conectar ao backend: {}", e);
            log_debug!("{}", error_msg);
            Err(error_msg)
        }
    }
...
pub async fn debug_backend_connection<R: Runtime>(app: AppHandle<R>) -> Result<String, String> {
    ...
    let server_url = match get_server_address(&app).await {
        ...
        Err(e) => {
            ...
            return Err(format!("Falha ao obter a URL do servidor: {}", e));
        }
    };
    ...
    match client.get(&test_url).send().await {
        ...
        Err(e) => {
            ...
            Err(format!("Falha na conexão com o backend: {}", e))
        }
    }
}
...
pub async fn open_external_url(url: String) -> Result<(), String> {
    ...
    match result {
        Ok(_) => Ok(()),
        Err(e) => Err(format!("Falha ao abrir a URL: {}", e)),
    }
}
...
pub async fn api_save_custom_openai_config<R: Runtime>(
    ...
    if endpoint.trim().is_empty() {
        return Err("A URL do endpoint é obrigatória".to_string());
    }
    if model.trim().is_empty() {
        return Err("O nome do modelo é obrigatório".to_string());
    }

    // Validate endpoint URL format
    if !endpoint.starts_with("http://") && !endpoint.starts_with("https://") {
        return Err("O endpoint deve começar com http:// ou https://".to_string());
    }

    // Validate optional numeric parameters
    if let Some(temp) = temperature {
        if !(0.0..=2.0).contains(&temp) {
            return Err("A temperatura deve estar entre 0.0 e 2.0".to_string());
        }
    }
    if let Some(top) = top_p {
        if !(0.0..=1.0).contains(&top) {
            return Err("O Top P deve estar entre 0.0 e 1.0".to_string());
        }
    }
    if let Some(tokens) = max_tokens {
        if tokens < 1 {
            return Err("O máximo de tokens deve ser pelo menos 1".to_string());
        }
    }
...
pub async fn api_test_custom_openai_connection<R: Runtime>(
    ...
    match request.send().await {
                                    ...
                                    if let Some(first_choice) = choices_array.get(0) {
                                        ...
                                        if has_message_structure {
                                            ...
                                            return Ok(serde_json::json!({
                                                "status": "success",
                                                "message": "Conexão bem-sucedida e resposta validada",
        ...
        Err(e) => {
            ...
            if e.is_timeout() {
                Err("Tempo de conexão esgotado. Verifique a URL do endpoint.".to_string())
            } else if e.is_connect() {
                Err("Não foi possível conectar ao endpoint. Verifique se a URL está correta e se o servidor está em execução.".to_string())
            } else {
