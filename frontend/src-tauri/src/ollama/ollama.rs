...
impl std::fmt::Display for OllamaError {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            OllamaError::Timeout => write!(f, "A solicitação expirou após 5 segundos. Verifique se o servidor do Ollama está em execução."),
            OllamaError::NetworkError(msg) => write!(f, "Erro de rede: {}. Verifique sua conexão e a URL do endpoint.", msg),
            OllamaError::InvalidEndpoint(msg) => write!(f, "Endpoint inválido: {}. Verifique o formato da URL.", msg),
            OllamaError::ServerError(msg) => write!(f, "Erro do servidor do Ollama: {}", msg),
            OllamaError::NoModelsFound => write!(f, "Nenhum modelo encontrado no servidor do Ollama. Baixe modelos usando 'ollama pull <model>'."),
            OllamaError::ParseError(msg) => write!(f, "Falha ao analisar a resposta do servidor: {}", msg),
        }
    }
}
        ...
        Ok(Err(http_err)) => {
            ...
            if is_localhost_endpoint(endpoint.as_deref()) {
                get_models_via_cli().map_err(|cli_err| {
                    format!("{}\n\nTambém tentei via CLI: {}", http_err, cli_err)
                })
            } else {
                Err(http_err)
            }
...
async fn get_models_via_http_with_retry(endpoint: Option<&str>) -> Result<Vec<OllamaModel>, String> {
    ...
    for attempt in 0..=MAX_RETRIES {
        match get_models_via_http_async(endpoint).await {
            ...
            Err(e) => {
                ...
                if e.contains("Endpoint inválido") || e.contains("404") {
                    return Err(e);
                }
    ...
    Err(format!("Falha após {} tentativas: {}", MAX_RETRIES, last_error))
}
...
async fn get_models_via_http_async(endpoint: Option<&str>) -> Result<Vec<OllamaModel>, String> {
    ...
    let response = client
        ...
        .map_err(|e| {
            if e.is_timeout() {
                OllamaError::NetworkError("Conexão expirou".to_string()).to_string()
            } else if e.is_connect() {
                OllamaError::NetworkError(format!("Não foi possível conectar a {}. Verifique se o servidor está em execução.", base_url)).to_string()
            } else {
    ...
    if !response.status().is_success() {
        return Err(OllamaError::ServerError(
            format!("HTTP {}: o servidor retornou um erro", response.status())
        ).to_string());
    }
...
fn get_models_via_cli() -> Result<Vec<OllamaModel>, String> {
    let output = Command::new("ollama")
        ...
        .map_err(|e| {
            OllamaError::NetworkError(
                format!("CLI do Ollama não encontrada ou não está no PATH: {}", e)
            ).to_string()
        })?;
    ...
    if !output.status.success() {
        ...
        return Err(OllamaError::ServerError(
            format!("Erro da CLI do Ollama: {}", stderr)
        ).to_string());
    }
...
pub async fn pull_ollama_model<R: Runtime>(
    ...
    // Check if model is already being downloaded
    {
        ...
        if downloading.contains(&model_name) {
            ...
            return Err(format!("O modelo {} já está sendo baixado", model_name));
        }
    }
    ...
    let response = client
        ...
        .map_err(|e| {
            if e.is_timeout() {
                format!("Download expirou. O modelo pode ser grande; tente usar a CLI do Ollama: ollama pull {}", model_name)
            } else if e.is_connect() {
                format!("Não foi possível conectar a {}. Verifique se o servidor Ollama está em execução.", base_url)
            } else {
                format!("Falha ao baixar o modelo: {}", e)
            }
        })?;
    ...
    if !response.status().is_success() {
        ...
        return Err(format!("Falha ao baixar o modelo (HTTP {}): {}", status, error_text));
    }
    ...
    while let Some(chunk) = stream.next().await {
        let chunk = chunk.map_err(|e| {
            let error_msg = format!("Falha ao ler o stream: {}", e);
        ...
        while let Some(newline_pos) = buffer.find('\n') {
            ...
            if let Ok(json) = serde_json::from_str::<serde_json::Value>(&line) {
                ...
                if let Some(error) = json.get("error").and_then(|v| v.as_str()) {
                    let error_msg = format!("Erro do Ollama: {}", error);
...
pub async fn delete_ollama_model(
    ...
    let response = client
        ...
        .map_err(|e| {
            if e.is_timeout() {
                format!("A solicitação de exclusão expirou para o modelo: {}", model_name)
            } else if e.is_connect() {
                format!("Não foi possível conectar a {}. Verifique se o servidor Ollama está em execução.", base_url)
            } else {
                format!("Falha ao excluir o modelo: {}", e)
            }
        })?;
    ...
    if !response.status().is_success() {
        let status = response.status();
        let error_text = response.text().await.unwrap_or_else(|_| "Erro desconhecido".to_string());
        return Err(format!("Falha ao excluir o modelo (HTTP {}): {}", status, error_text));
    }
